...
var getTargetLangs = function getTargetLangs() {
  var getLanguageURL = 'https://devapi.qordoba.com/v2/projects/' + projectId;
  var options = {
    method: 'GET',
    projectId: projectId,
    url: getLanguageURL,
  ...
  return (0, _requestPromise2.default)(options).then(function (body) {
    var targetLanguages = JSON.parse(body).project.target_languages;
    return targetLanguages.map(function (_ref) {
      var id = _ref.id,
          code = _ref.code;

      var lg = code.slice(0, 2);
      var langId = id;
      return { lg: lg, langId: langId };
    });
...
var getMilestoneId = function getMilestoneId() {
  var options = {
    ...
    headers: { 'x-auth-token': xAuthToken, consumerKey: consumerKey }
  };
  return (0, _requestPromise2.default)(options).then(function (body) {
    var _JSON$parse2 = JSON.parse(body),
        workflow = _JSON$parse2.workflow;

    var result = workflow.milestones.filter(function (milestone) {
...
var getQordobaTimestamps = function getQordobaTimestamps(languageId) {
  var getProjectFilesURL = 'https://app.qordoba.com/api/projects/' + projectId + '/languages/' + languageId + '/page_settings/search';
  var options = {
    method: 'POST',
    url: getProjectFilesURL,
    headers: { consumerKey: consumerKey, 'x-auth-token': xAuthToken },
    body: {},
    json: true
  };
  return (0, _requestPromise2.default)(options).then(function (body) {
    var obj = {};
    body.pages.forEach(function (_ref3) {
...
var syncTargetFiles = function syncTargetFiles() {
  ...
  _promise2.default.all([getTargetLangs(), getMilestoneId()]).then(function (result) {
    languages = result[0];
    milestoneId = result[1];
    return getAllQordobaTimestamps(languages, files);
  }).then(function (qTimestamps) {
    languages.forEach(function (_ref5) {
      var lg = _ref5.lg,
          langId = _ref5.langId;
